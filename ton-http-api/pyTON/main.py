...
from pytonlib.utils.tlb import Slice, MsgAddressInt, MsgAddress
...
from pytonlib.utils.address import (
    detect_address as __detect_address,
    prepare_address as _prepare_address,
)
...
tags_metadata = [
    ...
    {"name": "get config", "description": "Get blockchain config"},
    {
        "name": "run method",
...
async def startup():
    ...
    tonlib = TonlibManager(
        tonlib_settings=settings.tonlib,
        dispatcher=None,
    ...
    await asyncio.sleep(
        2
    )  # wait for manager to spawn all workers and report their status
...
async def validation_exception_handler(request, exc):
    ...
    return JSONResponse(
        res.dict(exclude_none=True), status_code=status.HTTP_422_UNPROCESSABLE_ENTITY
    )
...
async def timeout_exception_handler(request, exc):
    res = TonResponse(
        ok=False, error="Liteserver timeout", code=status.HTTP_504_GATEWAY_TIMEOUT
    )
    return JSONResponse(
        res.dict(exclude_none=True), status_code=status.HTTP_504_GATEWAY_TIMEOUT
    )
...
async def tonlib_error_result_exception_handler(request, exc):
    res = TonResponse(
        ok=False, error=str(exc), code=status.HTTP_500_INTERNAL_SERVER_ERROR
    )
    return JSONResponse(
        res.dict(exclude_none=True), status_code=status.HTTP_500_INTERNAL_SERVER_ERROR
    )
...
async def fastapi_generic_exception_handler(request, exc):
    res = TonResponse(
        ok=False, error=str(exc), code=status.HTTP_503_SERVICE_UNAVAILABLE
    )
    return JSONResponse(
        res.dict(exclude_none=True), status_code=status.HTTP_503_SERVICE_UNAVAILABLE
    )
...
def wrap_result(func):
    ...
    async def wrapper(*args, **kwargs):
        result = await asyncio.wait_for(
            func(*args, **kwargs), settings.tonlib.request_timeout
        )
    ...
    def g(func):
        ...
        def f(**kwargs):
            ...
            for k, v in sig.parameters.items():
                ...
                if (v.annotation is int or v.annotation is Optional[int]) and type(
                    kwargs[k]
                ) is str:
                    try:
...
async def get_wallet_information(
    ...
    res["balance"] = (
        result["balance"] if (result["balance"] and int(result["balance"]) > 0) else 0
    )
...
async def get_transactions(
    ...
    """
    address = prepare_address(address)
    return await tonlib.get_transactions(
        address,
        from_transaction_lt=lt,
        from_transaction_hash=hash,
        to_transaction_lt=to_lt,
        limit=limit,
        archival=archival,
    )
...
@wrap_result
async def get_masterchain_block_signatures(seqno: int):
    """
    Get up-to-date masterchain state.
    """
...
async def get_block_transactions(
    ...
    """
    return await tonlib.getBlockTransactions(
        workchain, shard, seqno, count, root_hash, file_hash, after_lt, after_hash
    )
...
@wrap_result
async def get_try_locate_tx(source: str, destination: str, created_lt: int):
    """
...
@wrap_result
async def get_try_locate_result_tx(source: str, destination: str, created_lt: int):
    """
...
@wrap_result
async def get_try_locate_source_tx(source: str, destination: str, created_lt: int):
    """
...
def int_is_for_null_addr(slice):
    slice.read_next(4)
    _ = MsgAddress.parse(slice)
    dest = MsgAddressInt(slice)
    if dest.workchain_id == 0 and dest.address == "0" * 64:
        return True


def body_is_for_null_addr(slice):
    try:
        cl_copy = deepcopy(slice)
        ref = Slice(cl_copy._refs[cl_copy._refs_offset])
        hashmap_cell = Cell()
        hashmap_cell.data.data = ref._data
        hashmap_cell.refs = [ref.copy() for ref in ref._refs]
        hashmap = {}
        parse_hashmap(hashmap_cell, 16, hashmap, bitarray())
        for k, v in hashmap.items():
            if int_is_for_null_addr(Slice(v.refs[0].copy())):
                return True
    except Exception as e:
        pass  # not highload

    while slice.refs_left() > 0:
        ref = slice.read_next_ref()
        if int_is_for_null_addr(ref):
            return True
    return False


def external_is_for_null_addr(slice):
    if slice.read_next(2) != bitarray("10"):
        return False
    has_init = slice.read_next(1)
    if has_init == bitarray("1"):
        return False
    body_in_subcell = slice.read_next(1)
    if body_in_subcell == bitarray("1"):
        body = slice.read_next_ref()
    else:
        body = slice
    return body_is_for_null_addr(body)
...
@wrap_result
async def send_cell(
    cell: Dict[str, Any] = Body(
        ..., embed=True, description="Cell serialized as object"
    )
):
    """
...
async def send_query(
    ...
    init_data: str = Body(
        default="", description="b64-encoded boc-serialized cell with init-data"
    ),
):
    """
    ...
    return await tonlib.raw_create_and_send_query(
        address, body, init_code=code, init_data=data
    )
...
async def send_query_cell(
    ...
    init_code: Optional[Dict[str, Any]] = Body(
        default=None,
        description='init-code cell as object: `{"data": {"b64": "...", "len": int }, "refs": [...subcells...]}`',
    ),
    init_data: Optional[Dict[str, Any]] = Body(
        default=None,
    ...
    return await tonlib.raw_create_and_send_query(
        address, body, init_code=qcode, init_data=qdata
    )
...
async def estimate_fee(
    ...
    code = codecs.decode(codecs.encode(init_code, "utf-8"), "base64")
    data = codecs.decode(codecs.encode(init_data, "utf-8"), "base64")
    return await tonlib.raw_estimate_fees(
        address, body, init_code=code, init_data=data, ignore_chksig=ignore_chksig
    )
...
async def estimate_fee_cell(
    ...
    try:
        ...
        raise HTTPException(status_code=400, detail="Error while parsing cell object")
    return await tonlib.raw_estimate_fees(
        address, body, init_code=qcode, init_data=qdata, ignore_chksig=ignore_chksig
    )
...
if settings.webserver.json_rpc:
    ...
    @app.post(
        "/jsonRPC",
        response_model=TonResponseJsonRPC,
        response_model_exclude_none=True,
        tags=["json rpc"],
    )
    async def jsonrpc_handler(
        json_rpc: TonRequestJsonRPC,
        request: Request,
        response: Response,
        background_tasks: BackgroundTasks,
    ):
