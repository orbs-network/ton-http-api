...
from pyTON.schemas import TonResponse, DeprecatedTonResponseJsonRPC, TonRequestJsonRPC
...
async def get_address_information(
    ...
    if "balance" in result and int(result["balance"]) < 0:
        result["balance"] = 0
    if (
        result["sync_utime"] < 1803189600
        and _detect_address(address)["raw_form"] in suspended_accounts
    ):
...
async def get_masterchain_block_signatures(
    seqno: int, tonlib: TonlibManager = Depends(tonlib_dep)
):
    """
    Get up-to-date masterchain state.
    """
...
if settings.webserver.json_rpc:
    ...
    @router.post(
        "/jsonRPC",
        response_model=DeprecatedTonResponseJsonRPC,
        response_model_exclude_none=True,
        tags=["json rpc"],
    )
    async def jsonrpc_handler(
        ...
        if not method in json_rpc_methods:
            ...
            return DeprecatedTonResponseJsonRPC(
                ok=False, error="Unknown method", id=_id
            )
        ...
        try:
            ...
            response.status_code = status.HTTP_422_UNPROCESSABLE_ENTITY
            return DeprecatedTonResponseJsonRPC(
                ok=False, error=f"TypeError: {e}", id=_id
            )
